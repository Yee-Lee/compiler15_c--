...
void printRegs(){
    int* realRegIsAllocated = g_intRegisterTable.isAllocated ;
    int realTableIndex;
    printf(" intReg[");
    for(realTableIndex= 0; realTableIndex < 5; ++realTableIndex)
    {
        printf(" %d", realRegIsAllocated[realTableIndex]);
    }

    printf("] floatReg[");

    realRegIsAllocated =  g_floatRegisterTable.isAllocated;
    for(realTableIndex= 0; realTableIndex < 6; ++realTableIndex)
    {
        printf(" %d", realRegIsAllocated[realTableIndex]);
    }
    printf("]\n");
